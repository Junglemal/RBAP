...
def create_job(session: Session) -> ParserJob:
    """создание статуса задачи парсера"""
...
def update_job(job_id: int, session: Session, **kwargs) -> None:
    """обновление статуса задачи парсера"""
...
def finish_job(job_id: int, session: Session, status: str, message: str | None = None) -> None:
    """указание статуса завершения работы парсера"""
    job = session.get(ParserJob, job_id)
    if not job:
        return
...
def get_last_job(session: Session) -> Optional[ParserJob]:
    """Получить последнюю задачу парсера"""
